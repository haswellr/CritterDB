...
function removePassivePerception(array){
    var output = [];
    for(var i=0;i<array.length;i++){
        var str = array[i];
        if(str.toLowerCase().indexOf("passive perception") == -1){
            output.push(str);
        }
    }
    return(output);
...
function parseAlignment(srdCreature){
    return(toTitleCase(srdCreature.alignment));
}
...
function parseSRDCreatureStats(srdCreature){
    var parsed = {
        ...
        alignment: parseAlignment(srdCreature),
        armorType: "",
        ...
        senses: removePassivePerception(convertCSVToArray(srdCreature.senses)),
        ...
        additionalAbilities: parseAbilities(srdCreature.special_abilities),
...
function parseSRDCreature(srdCreature,bestiaryId,publicBestiaryId){
    var parsed = {
    ...
    if(bestiaryId)
        parsed.bestiaryId = bestiaryId;
    if(publicBestiaryId)
        parsed.publishedBestiaryId = publicBestiaryId;
    return(parsed);
}
...
exports.parseSRDJsonPublic = function(req, res) {
    var creatures = req.body;
    var publicBestiaryId = req.params.publicBestiaryId;
    var parsed = [];
    console.log("--- parsing srd creatures ---");
    for(var i=0;i<creatures.length;i++){
        var creature = creatures[i];
        console.log("parsing: "+creature.name);
        var parsedCreature = parseSRDCreature(creature,undefined,publicBestiaryId);
        parsed.push(parsedCreature);
    }
    console.log("--- finished parsing ---");
    res.send(parsed);
